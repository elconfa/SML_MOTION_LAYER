...
**Base:** SML_v8
**Date:** 2026-07-23
**Scope:** rendere `FB_SmlAxisCtrl` un **superset di `FB_SML`** portando dentro
...
| **CSP + OTG** | ❌ | ✅ |
| eCmd + Ctrl/State + multi-asse + I_SmlAxis | ❌ | ✅ |

`FB_SmlAxisCtrl` copre ora tutto FB_SML **e in piu'** CSP/OTG, comando eCmd,
...
- Restano i caveat precedenti (enum non-strict; scaling velocita'=posizione;
  metodi I_SmlAxis come oggetti; MAPPING via flag runtime AXIS_MAP_ENABLE).

## Verifica
1. Build: `ST_CiA402_Status` risolto; `FB_SmlAxisCtrl` compila con le due nuove
